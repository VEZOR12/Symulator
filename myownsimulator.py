...
# POKEMON1'S ATTACKS
attacks1 = [
    Move(power=50, accuracy=70, type='Dark', category='Special', contact=False),
    Move(power=50, accuracy=70, type='Ghost', category='Special', contact=False),
    Move(power=50, accuracy=70, type='Ghost', category='Physical', contact=True),
    Move(power=50, accuracy=70, type='Water', category='Physical', contact=True, critical_chance=2)
]
# POKEMON2'S ATTACKS
attacks2 = [
    Move(power=50, accuracy=70, type='Psychic', category='Special', contact=False),
    Move(power=50, accuracy=70, type='Flying', category='Special', contact=False),
    Move(power=50, accuracy=70, type='Flying', category='Special', contact=False),
    Move(power=50, accuracy=70, type='Flying', category='Special', contact=False, critical_chance=2)
]
...
def attack(attacker, defender, runda):
    ...
    will_it_hit = chances_to_hit(attacker, defender, runda)
    print(will_it_hit)
...
def chances_to_hit(attacker, defender, runda):
    speed_factor = float(round(attacker.stats['spd'] / defender.stats['spd'], 4))
    if attacker.stats['spd'] > defender.stats['spd']:
        speed_factor = round(1 / speed_factor, 4)
    else:
        pass
    chance_to_hit = (speed_factor * 60 + 40) / 100
    if attacker.stats['spd'] > defender.stats['spd']:
        chance_to_hit = 1 - chance_to_hit + 1
    else:
        pass
    chance_to_hit = round(chance_to_hit * attacker.attacks[runda].accuracy)
    chance_to_hit = chance_to_hit * attacker.stats['pokemon_hit'] / defender.stats['pokemon_dodge']
    x = randint(1, 101)
    if x < chance_to_hit:
        will_it_hit = True
